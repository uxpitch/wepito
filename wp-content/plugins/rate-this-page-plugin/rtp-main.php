...
//		return rtp_extend_template( $display, $postid, $data, $ave_data );
		...
		if ( $data ) :
			foreach ( $data as $rows ) :
				$log_id = $rows->id;
				$user_type = ( $rows->user_type == 1 ) ? __( 'User', $pluginName ) : __( 'Guest', $pluginName );
				...
				$log_date = mysql2date(sprintf(__('%s @ %s', $pluginName), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', strtotime($rows->rate_date)));;
				$log_host_ip = $rows->ip . ' / ' . $rows->host;

				$rowsHtml .= "<tr valign='top'>

							<td> $user_type;  </td>
							<td>  $log_trustworthy;  </td>
							<td>  $log_objective;  </td>
							<td>  $log_complete;  </td>
							<td>  $log_wellwritten;  </td>
							<td>  $log_date;  </td>
						 </tr>";

			endforeach;
		else : $rowsHtml= "<td colspan='10' align='center'>No Results Found</td>";
		endif;


		$aft_options_array = get_option( 'aft_options_array' );
		$rtp_custom_labels = $aft_options_array['rtp_custom_labels'];
		...
		$rowsHtml ='
	     <table id="log-results" class="widefat tablesorter" style="text-align: center">
			<thead style="text-align: center">
					<tr>
							<th width="8%">User</th>
							<th width="9%">'. $lbl_trustworthy.'</th>
							<th width="9%">'. $lbl_objective.'</th>
							<th width="9%">'. $lbl_complete.'</th>
							<th width="9%">'.$lbl_wellwritten.'</th>
